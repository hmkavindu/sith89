...
// Citta data from script.js (first 89 cittas)
const cittasData = [
...
// Generate filename from citta name
function generateFilename(cittaName, id) {
    // Simplify name for filename
    const simplified = cittaName
        .replace(/\s+/g, '-')
        .replace(/[්ෘෙේෛොෝෞං ඃ]/g, '') // Remove diacritics
        .toLowerCase();
    return `citta-${id}-guide.html`;
}

// Generate HTML content for a citta
function generateCittaHTML(citta) {
    const isAkusala = citta.category === 'අකුසල්';
    const isKusala = citta.category === 'කුසල්';
    const colorScheme = isAkusala ? '#ef4444' : isKusala ? '#10b981' : '#3b82f6';
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background: linear-gradient(to bottom right, ${isAkusala ? '#fee2e2, #fed7aa' : isKusala ? '#dcfce7, #d1fae5' : '#dbeafe, #e0f2fe'});
            min-height: 100vh;
            padding: 20px;
            color: #1f2937;
        }
    ...
    <div class="container">
        ...
        <div class="section color-green">
            ...
            <div class="section-content" id="content-simple">
                <div class="content-box">
                    <h3>සරල භාෂාවෙන්:</h3>
                    <p>මෙම සිත ${citta.category} කාණ්ඩයට අයත් වන අතර ${citta.subcategory} සිතක් වේ.</p>
        ...
        <div class="section color-blue">
            ...
            <div class="section-content" id="content-details">
                <div class="content-box">
                    <h3>සිතෙහි ලක්ෂණ:</h3>
                    <p>මෙම සිත ${citta.category} වන අතර ${citta.subcategory} කාණ්ඩයට අයත් වේ.</p>
                    <div class="info-box">
        ...
        <div class="section" style="background: linear-gradient(to right, ${isAkusala ? '#fee2e2, #fed7aa' : isKusala ? '#dcfce7, #d1fae5' : '#dbeafe, #e0f2fe'}); border-left: 4px solid ${colorScheme};">
            ...
            <div class="section-content active">
                <div class="content-box">
                    <div class="info-box" style="background: ${isAkusala ? '#fee2e2' : isKusala ? '#dcfce7' : '#dbeafe'}; border-left-color: ${colorScheme};">
                        <p style="text-align: center; font-weight: 700; font-size: 18px; margin-bottom: 12px;">මතක තබා ගන්න:</p>
                        <p style="text-align: center;">${citta.name} යනු ${citta.category} ${citta.subcategory} සිතක් වන අතර චෛතසික ${citta.chaithasikaCount}ක් සමඟ ඇතිවේ.</p>
                    </div>
                </div>
            </div>
        </div>
...
// Generate pages for cittas 3-20 (skip 1 and 2 as they already exist)
console.log('Generating citta detail pages...\n');

let generated = 0;
let skipped = 0;

cittasData.forEach(citta => {
    const filename = generateFilename(citta.name, citta.id);
    const filepath = path.join(__dirname, filename);
    
    // Skip if already exists (cittas 1 and 2)
    if (citta.id === 1 || citta.id === 2) {
